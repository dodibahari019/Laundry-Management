...
class OrderController extends Controller
{
    ...
    public function detail(string $id_order)
    {
        ...
        $dataOrderStatusLogs = Orders::select([
            'logs.id_order_status_log',
            'logs.id_order',
            'logs.status',
            'logs.tanggal_ubah',
            'users.nama as user_name'
        ])
        ->join('tb_order_status_logs as logs', 'logs.id_order', '=', 'tb_orders.id_order')
        ->join('tb_users as users', 'logs.id_user', '=', 'users.id_user')
        ->where('tb_orders.id_order', $id_order)
        ->orderByDesc('logs.id_order_status_log')
        ->get();


        $currentlyDate = Carbon::now()->format('Y-m-d');

        return view('orders.detail', compact('dataOrder' ,'dataPelanggan', 'dataLayanan', 'currentlyDate'));
    }
