    ...
    public function store(Request $request)
    {
        try{
            ...
            if ($lastIdTransaksiPembayaran) {
                $lastNumberPem = (int)substr($lastIdTransaksiPembayaran->id_pembayaran, 3); // Ambil angka dari 'SUP001'
                $newNumber = $lastNumberPem + 1;
            } else {
                $newNumber = 1;
            }

            $newId = 'PAY' . str_pad($newNumber, 3, '0', STR_PAD_LEFT);

            Pembayaran::create([
                'id_pembayaran' => $validated['id_pembayaran'],
                'id_order' => $validated['id_order'],
                'metode' => $validated['metode'],
                'jumalah' => $validated['jumalah'],
