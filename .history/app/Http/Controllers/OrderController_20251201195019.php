...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        try{
            ...
            // Cari ID terakhir
            $lastIdTransaksiStatus = OrderStatusLog::orderBy('id_pembayaran', 'desc')->first();

            if ($lastIdTransaksiStatus) {
                $lastNumberPay = (int)substr($lastIdTransaksiStatus->id_pembayaran, 3); // Ambil angka dari 'SUP001'
                $newNumberPayPay = $lastNumberPay + 1;
            } else {
                $newNumberPayPay = 1;
            }
