...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        try{
            ...
            // Cari ID terakhir
            $lastIdTransaksiSatays = Pembayaran::orderBy('id_pembayaran', 'desc')->first();

            if ($lastIdTransaksiSatays) {
                $lastNumberPay = (int)substr($lastIdTransaksiSatays->id_pembayaran, 3); // Ambil angka dari 'SUP001'
                $newNumberPayPay = $lastNumberPay + 1;
            } else {
                $newNumberPayPay = 1;
            }
