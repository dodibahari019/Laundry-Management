    ...
    public function store(Request $request)
    {
        try{
            ...
            if ($lastIdTransaksiPembayaran) {
                ...
                $newNumberPayPay = $lastNumberPay + 1;
            } else {
                $newNumberPayPay = 1;
            }

            $newIdPay = 'PAY' . str_pad($newNumberPayPay, 3, '0', STR_PAD_LEFT);
            ...
            // Cari ID terakhir
            $lastIdTransaksiStatus = OrderStatusLog::orderBy('id_order_status_log', 'desc')->first();

            if ($lastIdTransaksiStatus) {
                $lastNumberLog = (int)substr($lastIdTransaksiStatus->id_order_status_log, 3); // Ambil angka dari 'SUP001'
                $newNumberLog = $lastNumberLog + 1;
            } else {
                $newNumberLog = 1;
            }

            $newIdPay = 'PAY' . str_pad($newNumberLog, 3, '0', STR_PAD_LEFT);
