...
class ReportController extends Controller
{
    ...
    public function generateReport(Request $request)
    {
        ...
        if ($jenisLaporan == 'transaksi') {
            ...
            $totalTransaksi = Orders::join('tb_pembayaran', 'tb_orders.id_order', '=', 'tb_pembayaran.id_order')
                ->select(
                    Orders::raw('COUNT(tb_orders.id_order) as jumlah_transaksi'),
                )
                ->whereBetween('tb_orders.tanggal_masuk', [$startDate, $endDate])
            ...
            SELECT SUM(x.total) AS total_pendapatan
            FROM tb_orders X
            INNER JOIN tb_pembayaran Y ON (x.id_order = y.id_order)
            WHERE 1=1

            $total = Orders::join('tb_pembayaran', 'tb_orders.id_order', '=', 'tb_pembayaran.id_order')
                ->select(
                    Orders::raw('DATE(tb_orders.tanggal_masuk) as tanggal_transaksi'),
                    Orders::raw('COUNT(tb_orders.id_order) as jumlah_transaksi'),
                    Orders::raw('SUM(tb_orders.total) as total_pendapatan'),
            ...
            SELECT AVG(x.total) AS rata_rata_per_transaksi 
            FROM tb_orders X
            INNER JOIN tb_pembayaran Y ON (x.id_order = y.id_order)
            WHERE 1=1

            SELECT COUNT(DISTINCT x.id_pelanggan)
            FROM tb_orders X
            INNER JOIN tb_pembayaran Y ON (x.id_order = y.id_order)
            WHERE 1=1
