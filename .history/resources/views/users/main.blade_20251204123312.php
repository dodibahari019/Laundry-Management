    ...
    <div class="bg-white rounded-2xl p-6 mb-6 border border-gray-100">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    <input type="text" placeholder="Cari username, nama, email..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-200 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Menampilkan {{ $jumlahSemua }} users, {{ $jumlahAdmin }} admin, {{ $jumlahKasir }} kasir, & {{ $jumlahPetugas }} petugas</p>
            </div>
        ...
        if (res.data && res.data.length > 0) {
            res.data.forEach((x, index) => {
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="py-4 px-6 font-semibold text-gray-900">{username }}</td>
                    <td class="py-4 px-6 text-gray-900">{nama }}</td>
                    <td class="py-4 px-6 text-gray-900">{email }}</td>
                    <td class="py-4 px-6">
                        @php
                            $roleUsers = $x->role;
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center justify-center space-x-2">
                            <button data-url="/users/{id_user }}/edit" class="modal-crud text-green-600 hover:text-green-800 p-1" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </button>
                            <form id="deleteForm{id_user }}" action="/layanan/{id_user }}" method="POST" style="display:none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <button onclick="confirmDelete('{id_user }}', this)" class="text-red-600 hover:text-red-800 p-1" title="Hapus">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
