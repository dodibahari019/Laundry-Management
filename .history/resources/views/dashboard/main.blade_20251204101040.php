...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 fade-in">
    ...
    <div class="bg-white rounded-2xl p-6 hover-scale border border-gray-100">
        ...
        <p id="targetRevenue" class="text-xs text-gray-400 mt-2">Target: Rp 0</p>
    </div>
</div>
...
{{-- Section 3: Status Order & Layanan --}}
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 fade-in">
    ...
    <div class="bg-white rounded-2xl p-6 border border-gray-100">
        ...
        <div class="grid grid-cols-2 gap-3 mt-4">
            <div class="text-center p-3 bg-yellow-50 rounded-lg">
                ...
                <p id="statusWaiting" class="text-2xl font-bold text-yellow-600">0</p>
            </div>
            <div class="text-center p-3 bg-blue-50 rounded-lg">
                <p class="text-xs text-gray-600 mb-1">Dicuci</p>
                <p id="statusWashing" class="text-2xl font-bold text-blue-600">0</p>
            </div>
            <div class="text-center p-3 bg-purple-50 rounded-lg">
                <p class="text-xs text-gray-600 mb-1">Setrika</p>
                <p id="statusIroning" class="text-2xl font-bold text-purple-600">0</p>
            </div>
            <div class="text-center p-3 bg-green-50 rounded-lg">
                <p class="text-xs text-gray-600 mb-1">Siap</p>
                <p id="statusReady" class="text-2xl font-bold text-green-600">0</p>
            </div>
        </div>
    </div>
...
{{-- Section 4: Antrian Order --}}
<div class="bg-white rounded-2xl p-6 border border-gray-100 mb-8 fade-in">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold text-gray-900">Antrian Order</h2>
        <span id="queueBadge" class="bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-bold">0 MENUNGGU</span>
    </div>
    ...
    <button onclick="window.location.href='/orders'" class="w-full mt-4 text-purple-600 font-semibold py-2 hover:bg-purple-50 rounded-lg transition">
        Lihat Semua Order →
    </button>
</div>

{{-- Section 5: Order Terbaru --}}
...
function updateCards(data) {
    ...
    $('#detailOrderProcess').text(`${data.cuciCount || 0} cuci • ${data.setrikaCount || 0} setrika`);
    ...
    $('#targetRevenue').text(`Target: ${formatRupiah(data.targetRevenue || 0)}`);
...
function updateCharts(data) {
    ...
    statusChart = new Chart(statusCtx, {
        type: 'doughnut',
        data: {
            labels: ['Menunggu', 'Dicuci', 'Setrika', 'Siap'],
            datasets: [{
                data: data.statusChartData || [0, 0, 0, 0],
        ...
        orders.slice(0, 5).forEach(x => {
            ...
            <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                <td class="py-3 px-4">
                    ...
                    <p class="text-xs text-gray-500">${formatTanggal(x.tanggal_masuk)}</p>
                </td>
                <td class="py-3 px-4">
