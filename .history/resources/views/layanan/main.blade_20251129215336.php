    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 border-t border-gray-200 flex items-center justify-between">
            <div class="text-sm text-gray-600">
                Menampilkan <span class="font-semibold">{{ $dataLayanan->firstItem() }}-{{ $dataLayanan->lastItem() }}</span> dari <span class="font-semibold">18</span> layanan
            </div>
...
<div class="p-6 border-t border-gray-200 flex items-center justify-between">
    <div class="text-sm text-gray-600">
        ...
        dari <span class="font-semibold">{{ $dataLayanan->total() }}</span> layanan
    </div>

    <!-- Letakkan kode pagination ini di sini -->
    <div class="flex space-x-2">
        @if($dataLayanan->onFirstPage())
            <button class="px-3 py-2 border rounded text-gray-600" disabled>Previous</button>
        @else
            <a href="{{ $dataLayanan->previousPageUrl() }}" class="px-3 py-2 border rounded text-gray-600 hover:bg-gray-50">Previous</a>
        @endif

        @foreach ($dataLayanan->getUrlRange(1, $dataLayanan->lastPage()) as $page => $url)
            <a href="{{ $url }}" class="px-3 py-2 rounded {{ $dataLayanan->currentPage() == $page ? 'bg-purple-600 text-white' : 'border text-gray-600 hover:bg-gray-50' }}">
                {{ $page }}
            </a>
        @endforeach

        @if($dataLayanan->hasMorePages())
            <a href="{{ $dataLayanan->nextPageUrl() }}" class="px-3 py-2 border rounded text-gray-600 hover:bg-gray-50">Next</a>
        @else
            <button class="px-3 py-2 border rounded text-gray-600" disabled>Next</button>
        @endif
    </div>
</div>
...
$(document).ready(function() {
    ...
    var table = $('#DataLayananTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "/layanan/getViewLayanan",
        columns: [
            { data: 'DT_RowIndex', className: 'text-left font-semibold text-gray-900' },
            { data: 'nama_layanan', className: 'text-left' },
            { data: 'badge_jenis', className: 'text-left', orderable: false, searchable: false },
            {
                data: 'harga',
                className: 'text-right font-bold',
                render: $.fn.dataTable.render.number('.', ',', 0, 'Rp ')
            },
            { data: 'durasi', className: 'text-left' },
            { data: 'badge_status', className: 'text-left', orderable: false, searchable: false },
