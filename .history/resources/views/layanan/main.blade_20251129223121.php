    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            res.data.forEach((x, index) => {
                ...
                let jam = x.durasi % 24;
                let jenisColor = x.jenis.toLowerCase() === 'kiloan'
...
<tr class="hover:bg-gray-50 transition">
    ...
    <td class="py-4 px-6">
        <div class="flex items-center justify-center space-x-2">
            <button onclick="openDetailModal(${x.id})" class="text-blue-600 hover:text-blue-800 p-1" title="Detail">Detail</button>
            <button onclick="openEditModal(${x.id})" class="text-green-600 hover:text-green-800 p-1" title="Edit">Edit</button>
            <button onclick="confirmDelete(${x.id})" class="text-red-600 hover:text-red-800 p-1" title="Hapus">Hapus</button>
        </div>
    </td>
</tr>
                `;
