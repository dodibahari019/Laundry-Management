    ...
    <div class="p-6 border-t border-gray-200 flex items-center justify-between">
        ...
        <div class="flex space-x-2">
            @if($dataLayanan->onFirstPage())
                <button disabled class="px-3 py-2 border rounded text-gray-600">Previous</button>
            @else
            ...
            @foreach ($dataLayanan->getUrlRange(1, $dataLayanan->lastPage()) as $page => $url)
                <a href="{{ $url }}" class="px-3 py-2 rounded {{ $dataLayanan->currentPage() == $page ? 'bg-purple-600 text-white' : 'border text-gray-600 hover:bg-gray-50' }}">{{ $page }}</a>
            @endforeach
            ...
            @else
                <button disabled class="px-3 py-2 border rounded text-gray-600">Next</button>
            @endif
        </div>
    </div>
