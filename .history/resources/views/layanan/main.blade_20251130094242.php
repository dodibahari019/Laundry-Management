    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-200 flex items-center justify-between">
            <div>
                ...
                <p id="jumlahLayananInfo" class="text-sm text-gray-500 mt-1">Menampilkan {{ $jumlahSemua }} layanan, {{ $jumlahAktif }} layanan aktif & {{ $jumlahNonAktif }} layanan nonaktif</p>
            </div>
...
function updateInfo(res) {
    ...
    $('#jumlahLayananInfo').html(
        `Menampilkan ${res.total} layanan, ${res.jumlahAktif} layanan aktif & ${res.jumlahNonAktif} layanan nonaktif`
    );
    ...
    if (res.current_page > 1) {
        paginationHtml += `<button data-page="${res.current_page - 1}" class="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50">Previous</button>`;
    } else {
        paginationHtml += `<button class="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50" disabled>Previous</button>`;
    }

    // Pages
    for (let page = 1; page <= res.last_page; page++) {
        paginationHtml += `<a href="{{ $url }}" class="px-3 py-2 rounded-lg text-sm font-semibold {{ $dataLayanan->currentPage() == $page ? 'bg-purple-600 text-white' : 'border text-gray-600 hover:bg-gray-50' }}">{{ $page }}</a>`;
    }
    ...
    if (res.current_page < res.last_page) {
        ...
        paginationHtml += `<button class="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50" disabled>Next</button>`;
    }
