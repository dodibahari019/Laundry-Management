...
@section('MainContentArea')
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-2xl p-6 border border-gray-100 hover-scale">
            ...
            <h3 id="cardTotalRata" class="text-2xl font-bold text-gray-900">Rp {{ shortNum($rataRata) }}</h3>
        </div>
    </div>
    ...
    function TampilkanLaporan() {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if(res.allTransaksi) {
                    $("#cardTotalTransaksi").text(shortNum(res.allTransaksi[0].jumlah_transaksi));
                }
                if(res.allPendapatan) {
                    $("#cardTotalPendapatan").text("Rp " + shortNum(res.allPendapatan[0].total_pendapatan));
                }
                if(res.allPelanggan) {
                    $("#cardTotalPelanggan").text(shortNum(res.allPelanggan[0].jumlah_transaksi));
                }
                if(res.allRataRata) {
                    $("#cardRataRata").text("Rp " + shortNum(res.allRataRata[0].rata_rata_per_transaksi));
                }
