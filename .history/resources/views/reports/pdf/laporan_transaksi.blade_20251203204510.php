...
<body>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">
                    <span class="small-">{{ rtrim(rtrim(number_format($beratQty, 2, ',', '.'), '0'), ',') }} {{ $satuan }}</span>
                </td>
                <td class="text-right font-bold">Rp {{ number_format($item->total, 0, ',', '.') }}</td>
                <td class="text-center">
                    <span class="small-">{{ strtoupper($item->status_order) }}</span>
                </td>
                <td class="text-center">{{ \Carbon\Carbon::parse($item->tanggal_selesai)->format('d/m/Y') }}</td>
                <td class="text-center">
                    <span class="small-">{{ strtoupper($item->metode) }}</span>
                </td>
    ...
    <div class="footer">
        <p><strong>*** Dokumen ini digenerate secara otomatis oleh sistem ***</strong></p>
        <p>Sistem Manajemen Laundry © {{ date('Y') }}</p>
    </div>
</body>
</html>