...
<script>
    ...
    function renderTableBody(jenis, data) {
        ...
        // Contoh menampilkan tanggal awal & akhir
        const tanggalAwal = res.tanggal_awal ? new Date(res.tanggal_awal) : null;
        const tanggalAkhir = res.tanggal_akhir ? new Date(res.tanggal_akhir) : null;

        if(tanggalAwal && tanggalAkhir){
            // Misalnya menaruh di elemen header
            document.getElementById('headerPeriode').innerText = 
                `Periode: ${formatTanggal(tanggalAwal)} s/d ${formatTanggal(tanggalAkhir)}`;
        }
