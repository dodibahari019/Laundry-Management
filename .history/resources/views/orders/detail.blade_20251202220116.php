    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <form id="idChangeStatusOrder" action="/orders/{{ $order->id_order }}/change" method="POST">
                @csrf
                <input readonly value="{{ $order->status_order }}" type="text" name="status_order" id="id_change_status_order" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
            </form>
            <button onclick="onclickCancelOrders();" id="id_button_change_status_order" type="button" class="px-6 py-3 gradient-primary text-white rounded-xl font-semibold hover:shadow-lg transition flex items-center space-x-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                </svg>
                <span>Simpan Sattus Order</span>
            </button>
        </div>
    </div>
