    ...
    function updateTableOrders(res) {
        ...
        if (res.data && res.data.length > 0) {
            res.data.forEach((x, index) => {
                ...
                let icon = x.status_order == 'diambil'? icon = '<svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                    </svg>';
                } else {
                    icon = '<svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    function fetchDataorders(page = 1) {
        ...
        timer = setTimeout(() => {
            ...
            $.ajax({
                url: '/orders/search',
                type: 'GET',
                data: { order: order, status: status, tanggal: tanggal,
                    // page: page
                 },
