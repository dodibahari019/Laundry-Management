...
<form action="/orders/{{ $orders->id_order }}" method="POST" id="idEditFormOrders" class="classEditForm p-6 space-y-6">
    @csrf
    @method('PUT')
    <!-- ID Order (Auto) -->
    <div class="bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded-xl border border-purple-200">
        <div class="flex items-center justify-between">
            <div>
                ...
                <input readonly hidden name="kode_order" value="{{ $orders->kode_order }}" id="id_kode_order" type="text" class="w-full px-2 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
         <div class="col-span-3">
            ...
            <select name="jenis_layanan" id="selectLayanan" onchange="ChooseSatuan(this)" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
                <option selected value="{{ $orders->id_layanan }}" data-layanan="{{$orders->nama_layanan}}" data-harga="{{$orders->harga}}" data-durasi="{{$orders->durasi}}" data-jenis="{{$orders->jenis}}">{{$orders->nama_layanan}} - Rp {{number_format($orders->harga,0,',' , '.')}}/{{$orders->jenis == 'kiloan'? 'Kg' : 'Pcs'}}</option>
        ...
        <div class="col-span-1">
            ...
            <input readonly name="berat" value="{{ $orders->berat }}" type="number" step="0.01" min="0" oninput="JustHitungTotal(this.value);" id="id_berat" placeholder="0" class="classRps w-full text-right pr-2 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
        </div>
        <div class="col-span-1">
            <label class="block text-sm font-medium text-gray-700 mb-2">
                <span id="labelQty">Qty (pcs)</span> *
            </label>
            <input readonly name="qty" value="{{$orders->jumlah}}" type="number" step="1" min="0" oninput="JustHitungTotal(this.value);" id="id_qty" placeholder="0" class="classRps w-full text-right pr-2 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <input type="date" value="{{$orders->tanggal_masuk}}" readonly name="tanggal_masuk" id="tanggalMasuk" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Estimasi Selesai *</label>
            <input type="date" value="{{$orders->tanggal_selesai}}" readonly name="tanggal_selesai" id="tanggalSelesai" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
        </div>
    </div>
    ...
    <div>
        ...
        <textarea rows="3" name="catatan" id="id_catatan" placeholder="Tambahkan catatan khusus (opsional)..." class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">{{ $orders->catatan }}</textarea>
    </div>
