   ...
   <div class="bg-white border border-gray-200 rounded-2xl p-6">
    ...
    <div class="p-6 border-t border-gray-200 flex items-center justify-between bg-white">
        ...
        <div class="flex space-x-3">
            ...
            <form id="cancelForm{{ $order->id_order }}" action="/orders/{{ $order->id_order }}/cancel" method="POST" style="display:none;">
                @csrf
                @method('PUT')
                <input name="kode_order" value="{{ $order->kode_order }}">
            </form>
            <button onclick="confirmCancel('{{ $order->id_order }}', this)" title="Cancel" class="px-6 py-3 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-xl font-semibold hover:shadow-lg transition flex items-center space-x-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
