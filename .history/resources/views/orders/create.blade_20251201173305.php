...
<form class="p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
         <div class="col-span-3">
            ...
            <select id="selectLayanan" onchange="ChooseSatuan()" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
                <option value="">-- Pilih Layanan --</option>
                @foreach($dataLayanan as $l)
                    <option value="{{$l->id_layanan}}" data-harga="{{$l->harga}}" data-jenis="{{$l->jenis}}">{{$l->nama_layanan}} - Rp {{number_format($l->harga,0,',' , '.')}}/{{$l->jenis == 'kiloan'? 'Kg' : 'Pcs'}}</option>
                @endforeach
            </select>
        </div>
        <div class="col-span-1">
            ...
            <input readonly value="0" name="berat" id="id_berat" type="text" placeholder="0" class="classRp w-full text-right pr-2 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
        </div>
        <div class="col-span-1">
            <label class="block text-sm font-medium text-gray-700 mb-2">
                <span id="labelQty">Qty (pcs)</span> *
            </label>
            <input readonly value="0" name="qty" id="id_qty" type="text" placeholder="0" class="classRp w-full text-right pr-2 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
        </div>
    </div>
